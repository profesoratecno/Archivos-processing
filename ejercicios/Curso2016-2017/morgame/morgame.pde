PImage backImg;// Imagen de fondo
PImage birdImg; // Imagen del jugador
PImage wallImg; // Palos
PImage startImg; // Imagen de inicio
...
void setup() {
  ...
  birdImg =loadImage("birdImg.png"); // Imagen del jugador
  wallImg =loadImage("wallImg.png"); // Palos
  startImg=loadImage("starimg.png"); // Imagen de inicio
}
...
void draw() {  // Se ejecuta 60 por segundo
  if (gamestate == 0) { // Si estamos en juego, se hará todo lo siguiente
    ...
    imageMode(CORNER);
    image(backImg, x, 0);
    image(backImg, x+backImg.width, 0); // Se suma la x para crear el desplazamient del fondo (*)
    x -= 6;
    vy += 1;
